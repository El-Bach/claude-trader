"""
EURUSD — Euro/US Dollar Specialist
APEX Capital AI

Strategy: Trend + Mean Reversion
- H4 trend using EMA 20/50/200 + ADX
- H1 pullback entries using Stochastic + Bollinger
- DXY inverse correlation from DOLLAR broadcast (mandatory)
- Session filter: London + NY overlap (Beirut time)
- Round number awareness: 1.0500, 1.0800, 1.1000, 1.1200
- News filter: ForexFactory (USD + EUR events)
...
STOCH_D        = 3
CANDLES_BACK   = 250
MAX_SPREAD     = 0.00020   # 2.0 pips max
MAGIC          = 20250401
...
# Key psychological levels for EURUSD
ROUND_LEVELS = [1.0200, 1.0300, 1.0400, 1.0500, 1.0600,
...
class EURUSDAgent:
    ...
    def receive_dollar_broadcast(self, broadcast: dict):
        ...
        # EURUSD is INVERSE to DXY
        # BULLISH_USD = bearish EURUSD bias
        # BEARISH_USD = bullish EURUSD bias
        impl = "BEARISH" if usd == "BULLISH_USD" else (
               "BULLISH" if usd == "BEARISH_USD" else "NEUTRAL")
        print(f"[{self.NAME}] Dollar signal: USD={usd} → EURUSD bias={impl}")
    ...
    def _nearest_round_level(self, price: float) -> dict:
        ...
        atr_proxy  = 0.0010   # approximate 1 ATR for context
    ...
    # ── Indicators (battle-tested pattern from gold.py) ───────────────
    ...
    def _dxy_text(self) -> str:
        ...
        # Translate for EURUSD (inverse)
        eur_impl = "BEARISH" if usd == "BULLISH_USD" else (
                   "BULLISH" if usd == "BEARISH_USD" else "NEUTRAL")
        return (f"USD Bias        : {usd}\n"
                f"DXY Trend       : {b.get('dxy_trend','FLAT')}\n"
                f"EURUSD Implication: {eur_impl} "
                f"(DXY and EURUSD are INVERSE)\n"
    ...
    def _ask_claude(self, h4, h1, m15, session,
        ...
        system_prompt = """You are EURUSD, a professional Euro/US Dollar forex trader at APEX Capital AI.
You specialise in EURUSD using a Trend + Mean Reversion strategy.
Capital protection is always top priority.

=== EURUSD TRADING RULES ===

RULE 1 - DXY IS YOUR COMPASS (MANDATORY):
  EURUSD moves INVERSE to DXY.
  - BULLISH_USD (DXY rising)  → only SHORT setups on EURUSD
  - BEARISH_USD (DXY falling) → only LONG setups on EURUSD
...
RULE 2 - H4 TREND FILTER:
  ...
  - ADX > 25: trending market → use trend following (EMA stack)
  - ADX 20-25: weak trend → reduce confidence by 10 points
  - ADX must be RISING (adx > adx_prev) — declining ADX = trend losing steam = reduce confidence 15

RULE 3 - ENTRY STRATEGIES:

  TRENDING MARKET (ADX > 25):
  - LONG: price > EMA20 > EMA50 > EMA200, RSI > 58 (strong momentum required)
  - SHORT: price < EMA20 < EMA50 < EMA200, RSI < 42 (strong momentum required)
  - RSI between 42-58 = momentum ambiguous = reduce confidence 15
  - Enter on pullbacks to EMA20, not on breakouts
  - Stochastic crossing up from oversold (<20) in uptrend = entry signal
  - Stochastic crossing down from overbought (>80) in downtrend = entry signal

  RANGING MARKET (ADX < 20):
  - LONG: price near lower Bollinger Band, Stochastic oversold (<20) and crossing up
  - SHORT: price near upper Bollinger Band, Stochastic overbought (>80) and crossing down
  - RSI divergence on H1 = high quality signal

RULE 4b - SMC CONFLUENCE:
  - Price inside H1 Bullish FVG OR at H1 Bullish OB -> valid BUY even if RSI is 50-58
  - Price inside H1 Bearish FVG OR at H1 Bearish OB -> valid SELL even if RSI is 42-50
  - FVG/OB = structural entry -- RSI threshold relaxed by 8 points when SMC present
  - Nearest Equal Highs above = primary TP target for BUY (liquidity sweep target)
  - Nearest Equal Lows below = primary TP target for SELL (liquidity sweep target)
...
RULE 4 - ROUND NUMBER AWARENESS:
  - Major levels: 1.0500, 1.0800, 1.1000, 1.1200
  - These act as strong support/resistance
  - Use as TP targets when price approaching
  - Avoid entries within 5 pips of round numbers (too much noise)
  - If price just broke through round number with volume: momentum trade

RULE 5 - SESSION RULES:
  - LONDON (best): cleanest EURUSD setups
  - OVERLAP_LONDON_NY (best): highest volume, most reliable
  - NEW_YORK: good but watch for fake moves at open
  - CLOSED: NO TRADE

RULE 6 - SPREAD FILTER:
  - Spread > 2.0 pips: NO TRADE (cost too high relative to move)

RULE 7 - CONFIDENCE & LOT:
  - Below 70%: WAIT
...
RULE 8 - SL/TP:
  - SL = 1.0-1.5x H4 ATR from entry (in price, not pips)
  - TP = minimum 2x SL distance
  - Use round numbers as TP targets when nearby
  - Minimum R:R = 1:2

RULE 9 - ALWAYS WAIT IF:
  - HIGH impact USD or EUR news within 30 min
  - Session CLOSED
  - Spread > 2.0 pips
  - DXY direction conflicts with trade direction
  - Price between H4 EMA20 and EMA50

RULE 10 - KEY PRICE LEVELS (Institutional Order Clusters):
...
RULE 11 - MULTI-TIMEFRAME ALIGNMENT (W1 + D1 — Master Filter):
...
RULE 11 - VWAP FILTER (Session Institutional Reference):
...
RULE 12 - VOLATILITY REGIME (ATR vs 20-day average):
...
RULE 13 - COT POSITIONING (CFTC Large Speculator Sentiment):
  COT data reflects institutional speculative positioning as of Tuesday each week.
  EUR futures: Long EUR futures = bullish EUR/USD.
...
Respond ONLY with valid JSON (no markdown, no backticks):
{
  ...
  "reasoning": "two sentence explanation of setup and key confirmations"
        ...
        user_prompt = f"""Analyse EURUSD and make a trade decision.
...
Stoch Signal: {'OVERSOLD (<20)' if h1['stoch_k'] < 20 else 'OVERBOUGHT (>80)' if h1['stoch_k'] > 80 else 'NEUTRAL'}
...
=== COT DATA (CFTC Large Speculator Positioning — EUR/USD futures) ===
{cot_text(cot)}

=== STRATEGIST EXECUTION PLAN (Daily Top-Down Analysis) ===
{self._strategy_text()}

=== UPCOMING NEWS (USD + EUR, next 2h) ===
{news_text}"""
    ...
    def analyse(self) -> dict | None:
        print(f"\n[{self.NAME}] Starting EURUSD analysis...")
        ...
        # Calculate intraday % for MANAGER tracking + fetch H1 raw data for SMC
