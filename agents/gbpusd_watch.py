"""
GBPUSD_WATCH — GBPUSD Position Monitor ("Cable Watch")
APEX Capital AI

Single-brain (Claude Sonnet) specialist watcher for open GBPUSD positions.
...
Key Cable characteristics:
  - Higher volatility than EURUSD — 40 pip spike threshold (vs 30 pips)
  - Deep, fast pullbacks in trends — adverse spikes may recover quickly
  - BoE sensitivity — news risk check includes GBP events
  - Round numbers (1.2000, 1.2500, 1.3000) act as strong S/R and TP magnets
"""
...
SYMBOL         = "GBPUSD"
PIP_SIZE       = 0.0001
NEWS_COUNTRIES = ["USD", "GBP"]

SPIKE_THRESHOLD    = float(os.getenv("SPIKE_GBPUSD", 0.0040))  # 40 pips
...
class GBPUSDWatch:
    ...
    def nearest_round_level(self, price: float) -> dict:
        ...
        return {
            ...
            "is_near":       distance < 0.0025,   # within 25 pips
            "above":         price > nearest,
        }
    ...
    def ask_claude(self, pos, ctx: dict, spike_size: float,
        ...
        if spike_type == "ADVERSE":
            spike_section = (
                ...
                f"\nNote: Cable makes deep pullbacks in trends. "
                f"A 40-60 pip adverse move can recover quickly in strong trends.\n"
                f"Decide: CLOSE now to take a smaller loss, "
            ...
            json_schema = (
                ...
                '  "reason": "one sentence — GBPUSD specific",\n'
                '  "confidence": 0-100\n'
                '}'
            )
            ...
            spike_section = (
                ...
                f"\nNote: Cable momentum moves can extend 80-120+ pips. "
                f"Trail SL+TP to ride if spike has genuine volume behind it.\n"
                f"Decide: MOVE_SL_TP to ride momentum, "
            ...
            spike_section = (
                ...
                f"News risk: {'YES - ' + news_desc if news_risk else 'No'}"
            )
            json_schema = (
                '{\n'
        ...
        system = f"""You are GBPUSD_WATCH, expert Cable (GBP/USD) position manager at APEX Capital AI.

GBPUSD KNOWLEDGE:
- Moves INVERSE to DXY (USD up = GBPUSD down)
- Round numbers (1.2000, 1.2500, 1.3000, 1.3500) act as strong S/R
- BoE vs Fed policy divergence is primary macro driver
- Cable makes DEEP pullbacks (50-70 pips) in strong trends — adverse moves can recover
- 40+ pip spike in 1 M1 candle = major event
- Higher ATR than EURUSD — wider SL needed, wider moves expected
...
{json_schema}"""
    ...
    def watch(self, pos) -> dict | None:
        ...
        if is_spike:
            ...
            dir_label   = "with us" if spike_type == "FAVORABLE" else "AGAINST us"
            trigger = (f"GBPUSD SPIKE {spike_size/PIP_SIZE:.1f} pips in 1 min "
                       f"({dir_label}) — {spike_type}")
        elif news_risk:
            spike_type = "NEWS"
            trigger    = f"NEWS RISK — {news_desc}"
